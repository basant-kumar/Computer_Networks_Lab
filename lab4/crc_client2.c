...
int compute_crc(char *data){
	printf("Enter the polynomial generator\n");
	char pg[100],rem[100],msg[100];
	read(0,pg,sizeof(pg));
	int size_pg,size_msg,size_rem,i,j,k,flag=0;
	size_pg=strlen(pg);
	...
	char rem1[size_pg];
	j=0;
	for(i=(size_msg-size_pg);i<size_rem;i++){
		rem1[j++]=rem[i];	
	}
...
/********************************************************************************************************/
	...
	printf("waiting for some data to receive\n");
	recv(sockfd,data,sizeof(data),0);
	printf("received message:%s\n",data);
	int flag=compute_crc(data);
	send(sockfd,&flag,sizeof(int),0);
	if(flag==0){
		printf("received data is currect\n");
	}else{   
		printf("received data is currupted\n");
	}
